...
function fieldLabel(header, index) {
    const text = header.trim();
    return text || `空字段 ${index + 1}`;
}
function mappingEntries() {
    return Object.entries(state.mapping)
        .map(([targetIndex, sourceIndex]) => [Number(targetIndex), Number(sourceIndex)])
        .filter(([targetIndex, sourceIndex]) => (Number.isInteger(targetIndex)
...
async function uploadAndRender(role) {
...
function visibleFieldCount(role) {
...
function updateFieldListHeight() {
...
function bindFieldEvents(item, role, index) {
    ...
    item.addEventListener("drop", (event) => {
        event.preventDefault();
        item.classList.remove("selected");
        const draggedRole = event.dataTransfer?.getData("role");
...
function mapFields(sourceIndex, targetIndex) {
...
function updateFieldSelection() {
    document.querySelectorAll(".field-item").forEach((item) => {
        ...
        item.style.removeProperty("--map-color");
    });
    const sourceToColor = new Map();
...
function applyMappedStyle(id, color) {
    const element = document.getElementById(id);
    if (!element) {
        return;
    }
...
async function generateFile() {
    ...
    try {
        ...
        window.setTimeout(() => {
            window.location.href = `/api/download?id=${encodeURIComponent(data.download_id)}`;
        }, 450);
    }
    catch (error) {
